...
| FR-26 | Seeded demo learners | FS §7 | T27 | `scripts/seed-demo-learners.ts` | Load < 2 s | 0:45 | 🟨 lightweight stand-in built 2026-09-25 (`npm run seed:demo`, 3 labelled-simulated learners via the real evidence pipeline) — **not the full T27 harness** (no `npm run seed:demo` load-time acceptance check run, no connection to the T25 simulated-learner conversation harness); good enough to populate the parent portal/learner card for a demo, not a substitute for T27 |
